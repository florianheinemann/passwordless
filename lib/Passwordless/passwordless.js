...
Passwordless.prototype.restricted = function(options) {
	...
	return function(req, res, next) {
		...
		if(options.failureRedirect) {
			...
			if(options.flashFailure) {
				if(!req.flash) {
					throw new Error('To use flashFailure, flash middleware is requied such as connect-flash');
				} else {
					req.flash('passwordless', options.flashFailure);
				}
			}	

			res.redirect(options.failureRedirect + queryParam);
		} else if(options.flashFailure) {
			throw new Error('flashFailure cannot be used without failureRedirect');
		} else {
